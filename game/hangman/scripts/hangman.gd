...
# Updates displayed result based on correct guesses on selected letters.
func update_result() -> void:
	var result = ""
	for c in word_list.chosen_word:
		if c == " ":
			result += " "
		else:
			result += "_"
	for target in correct_chars:
		for index in range(word_list.chosen_word.length()):
			if target == word_list.chosen_word[index]:
				result[index] = target
	result_label.text = result

# Updates the display of text for wrong guesses when selecting a letter.
...
func start_game() -> void:
	if word_list.words.size() < 1:
		word_list.chosen_word = "PLACEHOLDER"
	else:
		word_list.set_chosen_word()
	attempt = 0
	chars = get_chars()
	correct_chars = []
	wrong_chars = []
	...
	wrong5.text = ""
	wrong6.text = ""
	guess_word.max_length = word_list.chosen_word.length()
	guess_row.show()
	character_row.show()
	update_hangman_status()
	update_wrong_chars()
...
func _on_guess_pressed() -> void:
	if guess_word.text.to_upper() == word_list.chosen_word.to_upper():
		correct_chars = chars
		chars.clear()
	else:
		attempt += 1
...
func _check_win_or_lose_condition() -> void:
	if chars.size() == 0:
		result_label.text = "You Win!\nThe correct word was: " + word_list.chosen_word
		guess_row.hide()
		character_row.hide()
	if attempt == 6:
